...
def test_org_mode_docs_examples(base):
    assert render({'day': 5, 'base': 'current', 'year': 3, 'month': 2, 'type': 'absolute'}, base) == date(year=3, month=2, day=5)
    assert render({'day': 2, 'base': 'current', 'year': 3, 'month': 5, 'type': 'absolute'}, base) == date(year=3, month=5, day=2)
    ...
    assert render({'day': 12, 'base': 'current', 'year': 9, 'month': 9, 'type': 'absolute'}, base) == base + relativedelta(day=12, month=9, year=9)
    ...
    assert render({'base': 'current', 'weeks': 1, 'type': 'past', 'weekday': 2}, base) == base - relativedelta(weekday=calendar.WEDNESDAY)
